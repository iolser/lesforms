...
def create_event(request):
	# if not Proponent.objects.filter(generaluser_ptr=request.user.id).exists() or not Administrator.objects.filter(generaluser_ptr=request.user.id).exists():
	# 	return redirect('/')
	if request.method == 'POST':
		form = EventForm(request.POST)

		if form.is_valid():
			form.instance.organizer = Proponent.objects.get(generaluser_ptr__user_ptr_id=get_user(request).id)
			form.save()
...
def event_logistics_form(request, id):
	if not Event.objects.get(id=id, organizer__generaluser_ptr__user_ptr=request.user) and not Administrator.objects.filter(generaluser_ptr=request.user.id).exists():
		return redirect("/")
	event = Event.objects.get(id=id)
...
# if not Event.objects.filter(id=id, organizer=request.user.id).exists() or not Administrator.objects.filter(generaluser_ptr=request.user.id).exists():
# 	return redirect('/')
	event = Event.objects.get(id=id)
...
def attrib_equipment(request, id):
	...
	return render(request, 'logistic_validate_equipment.html', {'form' : form, 'requestequip': requestequip, 'items' : items})
...
def attrib_service(request, id):
	...
	for serviceuse in allservices:
		if services.filter(id=serviceuse.item.id).exists() :
			services = services.exclude(id=serviceuse.item.id)
	...
	return render(request, 'logistic_validate_service.html', {'form' : form, 'requestserv': requestserv, 'services' : services})
...
def attrib_location(request, id):
	...
	for locationuse in alllocations:
		if locations.filter(id=locationuse.item.id).exists() :
			locations = locations.exclude(id=locationuse.item.id)
	...
	return render(request, 'logistic_validate_location.html', {'form' : form, 'requestlocation': requestlocation, 'locations' : locations})
